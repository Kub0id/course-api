...
async function validateDataTypes(body, fields, context, method, path, headers, response, requestBody) {
    fields.forEach(field => {
        try{
            switch(field.type){
                case 'string':
                    expect(getNestedValue(field.path, body)).to.be.string()
                    break
                case 'number':
                    expect(getNestedValue(field.path, body)).to.be.number()
                    break 
                default:
            ...
            addRequestInfoToReport(context, method, path, headers, response, requestBody)
            const value = getNestedValue(field.path, body)
            assert.fail(value, field.type, `${field.path} expected value type is ${field.value}, but actual was ${value}`)
        }
    })
}